...
plot(chain) # Rough way to verify convergence
summary(chain)
autocorr.diag(chain) # This is the correlation between sampled parameter values and thinned chain
effectiveSize(chain) # If some correlation, effective sample size will be smaller than the input sample size

# Formal testing of chain convergence
geweke.diag(chain, frac1 = 0.1, frac2 = 0.5) # Geweke's convergence diagnostic. Reports z-scores. For alpha = 0.05, reject Ho if z<-1.96 or z>1.96
heidel.diag(chain, eps = 0.1, pvalue = 0.05) # Heidelberg & Welch's convergence diagnostic
